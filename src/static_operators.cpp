    ...
    const std::span<const Tetrahedron> source_tetrahedra)
{
    ...
    for (std::size_t source = 0; source < source_positions.size(); ++source) {
        ...
        for (int mode = 0; mode < basis.size(); ++mode) {
            ...
            for (int component = 0; component < 3; ++component) {
                const double value = green_factor * averaged_gradient[component];
                if (value != 0.0) {
                    result.entries.push_back({
                        mode,
                        static_cast<int>(3 * source) + component,
    ...
    const TargetModel target_model)
{
    ...
    // Validate indices and construct target-row metadata before evaluating
    // geometry.  The sequential build also lets analytical tetrahedron
    // kernels report construction-time domain errors safely.
    ...
    for (std::size_t index = 0; index < sorted.size(); ++index) {
        ...
        if (effective_source_geometry == SourceGeometry::PointDipole &&
            ...
            radius_squared == 0.0) {
            // Preserve the singular block.  An explicit identity map skips
            // only blocks marked by the interaction; coincident distinct
            // particles consequently retain the established NaN behaviour.
