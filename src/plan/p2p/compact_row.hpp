 ...
 * own allocation and iteration order; both the ordinary plan builder in
 * `plan/p2p/compact.cpp` and the fused cache decode in `cache/format.cpp`
 * call this per row so the mapping is defined once. `compact_plan`'s arrays
 * must already be sized to hold `slot`.
 */
